...
def test_sync_item_adds_label_from_movie_tag():
    ...
    sync = make_sync(FakePlex([item]), radarr_entries=[({"tmdb:603"}, ["kids"])])
...
def test_show_matched_by_tmdb_when_plex_has_no_tvdb():
    ...
    sonarr = [({"tvdb:121361", "tmdb:2316"}, ["family"])]
...
def test_sync_by_ids_finds_and_labels_item():
    item = FakePlexItem("Dune", "movie", labels=set(), guid_keys={"tmdb:603"})
    sync = make_sync(FakePlex([item]), radarr_entries=[({"tmdb:603"}, ["family"])])
...
def test_sweep_processes_all_items_and_counts_changes():
    ...
    sync = make_sync(
        FakePlex(items),
        radarr_entries=[({"tmdb:603"}, ["kids"]), ({"tmdb:2"}, [])],
        sonarr_entries=[({"tmdb:2316"}, ["family"])],
    )
    summary = sync.sweep()
...
def test_dry_run_does_not_mutate():
    item = FakePlexItem("Dune", "movie", labels=set(), guid_keys={"tmdb:603"})
    sync = make_sync(FakePlex([item]), radarr_entries=[({"tmdb:603"}, ["kids"])], dry_run=True)
    assert sync.sync_item(item) is True
    assert item.labels() == set()
...
def test_sweep_one_added_message_grouped_by_type_and_label():
    ...
    sync = _labeled_sync(
        FakePlex(items),
        [({"tmdb:1"}, ["kids"]), ({"tmdb:2"}, ["family"])],
        notifier,
        StateStore(":memory:"),
        sonarr_entries=[({"tvdb:3"}, ["kids"])],
    )
    sync.sweep()
...
def test_sweep_lists_every_item_no_truncation():
    ...
    sync = _labeled_sync(
        FakePlex(items),
        [({f"tmdb:{i}"}, ["kids"]) for i in range(40)],
        notifier,
        StateStore(":memory:"),
    )
    sync.sweep()
...
def test_label_notification_deduped_per_item_label():
    ...
    sync = _labeled_sync(FakePlex([item]), [({"tmdb:1"}, ["kids"])], notifier, state)
    sync.sweep()
...
def test_webhook_sync_sends_single_label_notification():
    ...
    notifier = FakeNotifier()
    sync = _labeled_sync(FakePlex([item]), [({"tmdb:1"}, ["kids"])], notifier, StateStore(":memory:"))
...
def test_removal_fires_notification():
    ...
    _labeled_sync(plex, [({"tmdb:1"}, ["kids"])], notifier, state).sweep()
...
def test_no_label_notification_in_dry_run():
    ...
    notifier = FakeNotifier()
    sync = _labeled_sync(
        FakePlex([item]), [({"tmdb:1"}, ["kids"])], notifier, StateStore(":memory:"), dry_run=True
    )
    sync.sweep()
    assert notifier.sent == []
